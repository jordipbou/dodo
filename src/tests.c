...
#define free_nodes(x)			((x->size - sizeof(X)) / (2*sizeof(C)) - 1)
...
void test_block_initialization() {
	...
	TEST_ASSERT_EQUAL_INT(free_nodes(x), height(F(x)));
	...
	TEST_ASSERT_EQUAL_INT(TOP(x), x->nodes);
...
void test_push() {
	...
	TEST_ASSERT(IS(ATOM, D_(S(x))));
	TEST_ASSERT_EQUAL_INT(13, A(D_(S(x))));

	while (height(F(x)) > 0) { push(x, ATOM, 7); }
...
void test_join_atom_atom_2() {
	...
	TEST_ASSERT_EQUAL_INT(7, A(A(S(x))));
	TEST_ASSERT_EQUAL_INT(13, A(D_(A(S(x)))));
	TEST_ASSERT_EQUAL_INT(17, A(D_(S(x))));
}
...
void test_join_atom_list_1() {
	...
	TEST_ASSERT_EQUAL_INT(7, A(A(S(x))));
	TEST_ASSERT_EQUAL_INT(11, A(D_(A(S(x)))));
	TEST_ASSERT_EQUAL_INT(13, A(D_(D_(A(S(x))))));
}
