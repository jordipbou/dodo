...
void test_pair_types() {
	...
	p->next = TYPED(p2, T_ATOM);

	TEST_ASSERT_EQUAL_PTR(p2, NEXT(p));
	TEST_ASSERT_EQUAL_INT(T_ATOM, TYPE(p));
}
...
void test_block_initialization() {
	...
	for (PAIR* p = ctx->there; p <= ctx->top; p++) {
		TEST_ASSERT_EQUAL_INT(T_FREE, TYPE(p));
	}
}
...
void test_alloc() {
	...
	TEST_ASSERT_EQUAL_INT(free_nodes, depth(ctx->free));
	PAIR* p = alloc(ctx, 13, T_WORD, NIL);
	...
	while (depth(ctx->free) > 0) { nalloc(ctx, 1, NIL); }

	TEST_ASSERT_EQUAL_INT(0, depth(ctx->free));
	PAIR* p3 = nalloc(ctx, 21, NIL);
...
void test_reclaim() {
	...
	TEST_ASSERT_EQUAL_INT(free_nodes, depth(ctx->free));
	PAIR* p = nalloc(ctx, 21, nalloc(ctx, 17, nalloc(ctx, 13, NIL)));
	...
	p = lalloc(ctx, nalloc(ctx, 13, nalloc(ctx, 17, NIL)), nalloc(ctx, 21, NIL));
	TEST_ASSERT_EQUAL_INT(2, depth(p));
...
void test_inner_interpreter() {
	...
	ctx->ip = nalloc(ctx, 13, nalloc(ctx, 7, NIL));
	inner(ctx);
	...
	ctx->ip = alloc(ctx, (CELL)&_add, T_PRIM, NIL);
	inner(ctx);
