...
#define IS(type, pair)					(TYPE(pair) == type)
#define REF(pair)								(IS(LIST, pair) || IS(XT, pair))
...
CELL execute(CTX* ctx, CELL lambda) {
	CELL result;
	ctx->ip = lambda;
	do {
		if (ctx->ip == 0) {
		...
		switch (TYPE(ctx->ip)) {
			case ATOM: 
				PUSH(ctx, CAR(ctx->ip), ATOM, ctx->stack); 
				ctx->ip = NEXT(ctx->ip); 
				break;
			case LIST:
				PUSH(ctx, clone(ctx, CAR(ctx->ip)), LIST, ctx->stack);
				ctx->ip = NEXT(ctx->ip);
				break;
			case PRIMITIVE:
				((FUNC)CAR(ctx->ip))(ctx);
				break;
			case XT:
				if (REF(CAR(ctx->ip)) && CDR(ctx->ip) != 0) {
					PUSH(ctx, ctx->ip, ATOM, ctx->rstack);
				}
