...
//	if (ctx->free == ctx->there 
//	|| NEXT(ctx->free) == ctx->there 
//	|| NEXT(NEXT(ctx->free)) == ctx->there 
//	|| NEXT(NEXT(NEXT(ctx->free))) == ctx->there) {
...
//	str[len] = 0;
//
//	ctx->compiled = cons(ctx, (CELL)str, AS(ATOM, ctx->compiled));
//	ctx->compiled = cons(ctx, len, AS(ATOM, ctx->compiled));
//
//	return 0;
//}
//
